...
# Uses mock labels to ensure training data has labels matching test addresses
...
# Use mock labels for testing - generates labels from address_features
export LABEL_SOURCES="mock"

log_info "=== Phase 3: Batch Processing ==="
log_info "PostgreSQL: $POSTGRES_HOST:$POSTGRES_PORT → Hudi + Neo4j"
log_info "Label source: mock (generates labels from test addresses)"
...
run_batch_job() {
    ...
    local output
    if [ -n "$extra_env" ]; then
        if ! output=$(eval "$extra_env make batch-$job" 2>&1); then
...
# Use mock labels - must run after features so addresses exist
run_batch_job "labels" "[3/5] Labels: Generate mock labels from test addresses" "LABEL_SOURCES=mock"
...
./scripts/trino-query.sh "
SELECT label_type, COUNT(*) as cnt FROM hudi.chainrisk.address_labels GROUP BY label_type
" 2>/dev/null | grep -v "^--" | tail -n +2 || log_warn "Trino query failed"

# Verify training dataset has labels
log_info ""
log_info "=== Training Dataset Label Distribution ==="
./scripts/trino-query.sh "
SELECT label, COUNT(*) as cnt FROM hudi.chainrisk.training_dataset GROUP BY label ORDER BY cnt DESC LIMIT 10
" 2>/dev/null | grep -v "^--" | tail -n +2 || log_warn "Trino query failed"
