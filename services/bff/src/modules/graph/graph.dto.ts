...
export class GetNeighborsQueryDto {
  ...
  @ApiPropertyOptional({ description: 'Maximum number of neighbors to return', default: 50 })
  @Type(() => Number)
  @IsInt()
  @Min(1)
  @Max(200)
...
export class GetHighRiskQueryDto {
  @ApiPropertyOptional({ description: 'Risk score threshold (0.0 - 1.0)', default: 0.6 })
  @Type(() => Number)
  @IsNumber()
  @Min(0)
  @Max(1)
...
export class AddressInfoResponse {
  ...
  @ApiPropertyOptional({ description: 'Tags associated with the address', type: [String] })
  tags?: string[];

  @ApiPropertyOptional({ description: 'Cluster ID if address belongs to a cluster' })
  clusterId?: string;
...
export class NeighborInfo {
  ...
  @ApiPropertyOptional({ description: 'Direction of transfers (IN, OUT, BOTH)' })
  direction?: string;
...
export class SyncStatusResponse {
  @ApiProperty({ description: 'Sync status (IDLE, RUNNING, COMPLETED, FAILED)' })
  status: string;
