...
export class RiskController {
  ...
  @ApiResponse({ status: 200, description: 'Risk score calculated', type: RiskScoreResponse })
  ...
  @ApiResponse({ status: 200, description: 'Risk scores calculated', type: BatchRiskScoreResponse })
  @ApiResponse({ status: 401, description: 'Unauthorized - Missing Gateway headers' })
  @ApiResponse({ status: 400, description: 'Bad request' })
  async scoreAddressesBatch(
    ...
    logger.debug('Batch scoring addresses', { count: dto.addresses.length, userId: user.sub });
    return this.riskService.scoreAddressesBatch(dto);
  }

  @Get('rules')
  @ApiOperation({ summary: 'Get all risk rules' })
  @ApiResponse({ status: 200, description: 'Risk rules list', type: [RiskRule] })
  @ApiResponse({ status: 401, description: 'Unauthorized - Missing Gateway headers' })
