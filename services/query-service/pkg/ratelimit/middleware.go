...
// RouteRateLimiter provides route-specific rate limiting
type RouteRateLimiter struct {
	limiters map[string]*RateLimiter
	mu       sync.RWMutex
}

// RouteConfig defines rate limit for a specific route pattern
...
func NewRouteRateLimiter(configs []RouteConfig) *RouteRateLimiter {
	rrl := &RouteRateLimiter{
		limiters: make(map[string]*RateLimiter),
	}

	for _, cfg := range configs {
		rrl.limiters[cfg.Pattern] = NewWithConfig(Config{
			RequestsPerMinute: cfg.RequestsPerMinute,
			BurstSize:         cfg.RequestsPerMinute / 5,
			CleanupInterval:   5 * time.Minute,
			MaxEntries:        10000,
		})
	}
...
// GetMiddleware returns middleware for a specific route pattern
func (rrl *RouteRateLimiter) GetMiddleware(pattern string) gin.HandlerFunc {
	rrl.mu.RLock()
	limiter, ok := rrl.limiters[pattern]
	rrl.mu.RUnlock()

	if !ok {
		// Return no-op middleware if pattern not configured
