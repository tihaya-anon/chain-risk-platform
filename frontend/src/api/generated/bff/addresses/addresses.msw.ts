...
export const getAddressControllerGetAddressTransfersMockHandler = (overrideResponse?: PaginatedTransfersResponse | ((info: Parameters<Parameters<typeof http.get>[1]>[0]) => Promise<PaginatedTransfersResponse> | PaginatedTransfersResponse), options?: RequestHandlerOptions) => {
  return http.get('*/api/v1/addresses/:address/transfers', async (info) => {await delay(1000);
  
    return new HttpResponse(JSON.stringify(overrideResponse !== undefined
    ? (typeof overrideResponse === "function" ? await overrideResponse(info) : overrideResponse)
    : getAddressControllerGetAddressTransfersResponseMock()),
      { status: 200,
        headers: { 'Content-Type': 'application/json' }
      })
  }, options)
...
export const getAddressControllerGetAddressStatsMockHandler = (overrideResponse?: AddressStatsResponse | ((info: Parameters<Parameters<typeof http.get>[1]>[0]) => Promise<AddressStatsResponse> | AddressStatsResponse), options?: RequestHandlerOptions) => {
  return http.get('*/api/v1/addresses/:address/stats', async (info) => {await delay(1000);
  
    return new HttpResponse(JSON.stringify(overrideResponse !== undefined
    ? (typeof overrideResponse === "function" ? await overrideResponse(info) : overrideResponse)
    : getAddressControllerGetAddressStatsResponseMock()),
      { status: 200,
        headers: { 'Content-Type': 'application/json' }
      })
  }, options)
...
export const getAddressesMock = () => [
  getAddressControllerGetAddressInfoMockHandler(),
  getAddressControllerGetAddressTransfersMockHandler(),
  getAddressControllerGetAddressStatsMockHandler()
]
