...
export function AdminPage() {
  ...
  const syncStatusQuery = useQuery({
    ...
    refetchInterval: 5000, // Refresh every 5s when on admin page
  })
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-6">
        ...
        <div>
          ...
          <p className="text-gray-600 mt-1">
            Manage data synchronization, clustering, and tag propagation
          </p>
        </div>
          ...
          >
            <div className="space-y-4">
              ...
              <Button
                ...
                loading={runClusteringMutation.isPending}
                className="w-full"
              >
              ...
              {runClusteringMutation.data && (
                <div className="p-4 bg-green-50 border border-green-200 rounded-lg">
                  <p className="font-medium text-green-800">Clustering Complete</p>
                  <div className="mt-2 text-sm text-green-700 space-y-1">
                    <p>Clusters created: {runClusteringMutation.data.clustersCreated}</p>
                    <p>
                      Addresses clustered: {runClusteringMutation.data.addressesClustered}
                    </p>
                    <p>Duration: {runClusteringMutation.data.durationMs}ms</p>
                  </div>
                </div>
              )}
            </div>
          ...
          >
            <div className="space-y-4">
              ...
              <Button
                ...
                loading={manualClusterMutation.isPending}
              ...
              {manualClusterMutation.data && (
                <div className="p-4 bg-green-50 border border-green-200 rounded-lg">
                  <p className="font-medium text-green-800">Cluster Created</p>
                  <div className="mt-2 text-sm text-green-700 space-y-1">
                    <p>Clusters created: {manualClusterMutation.data.clustersCreated}</p>
                    <p>
                      Addresses clustered: {manualClusterMutation.data.addressesClustered}
                    </p>
                  </div>
                </div>
              )}

              {manualClusterMutation.error && (
                <div className="p-4 bg-red-50 border border-red-200 rounded-lg">
                  <p className="font-medium text-red-800">Error</p>
                  <p className="text-sm text-red-700 mt-1">
                    {(manualClusterMutation.error as Error).message}
                  </p>
                </div>
              )}
            </div>
        ...
        <Card title="Tag Propagation" subtitle="Propagate risk tags through the graph">
          <div className="space-y-4">
            <p className="text-sm text-gray-600">
              ...
              propagated with a decay factor, meaning neighbors receive a reduced risk
              score based on their distance from the source.
            </p>
            ...
            <Button
              onClick={() => propagateTagsMutation.mutate()}
              loading={propagateTagsMutation.isPending}
              variant="secondary"
              className="w-full"
            >
