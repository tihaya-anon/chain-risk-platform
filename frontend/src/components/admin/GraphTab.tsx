...
interface GraphTabProps {
  ...
  runClustering: { mutate: () => void; isPending: boolean; data?: { clustersCreated?: number; addressesClustered?: number; durationMs?: number } }
  manualCluster: { mutate: (params: { data: { addresses: string[] } }) => void; isPending: boolean; data?: { clustersCreated?: number; addressesClustered?: number }; error?: Error }
  propagateTags: { mutate: () => void; isPending: boolean; data?: { addressesAffected?: number; tagsPropagated?: number; durationMs?: number } }
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="space-y-6">
          <div className="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <span className={`inline-block mt-1 px-2 py-0.5 text-xs font-medium rounded-full ${getStatusColor(status?.status)}`}>
                  {status?.status || "UNKNOWN"}
                </span>
              </div>
            </div>
            <Button onClick={() => triggerSync.mutate()} loading={triggerSync.isPending} disabled={status?.status === "RUNNING"}>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div className="p-4 bg-blue-50 rounded-lg">
              ...
              <p className="text-2xl font-bold text-blue-900">{status?.totalAddresses?.toLocaleString() || "-"}</p>
            </div>
            <div className="p-4 bg-purple-50 rounded-lg">
              ...
              <p className="text-2xl font-bold text-purple-900">{status?.totalTransfers?.toLocaleString() || "-"}</p>
            </div>
            <div className="p-4 bg-orange-50 rounded-lg">
              ...
              <p className="text-2xl font-bold text-orange-900">{status?.lastSyncedBlock?.toLocaleString() || "-"}</p>
            </div>
            <div className="p-4 bg-green-50 rounded-lg">
              ...
              <p className="text-2xl font-bold text-green-900">{status?.network || "-"}</p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
            ...
            <div className="flex justify-between p-3 bg-gray-50 rounded">
              ...
              <span className="font-medium">{formatTime(status?.nextSyncTime)}</span>
            </div>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Card title="Automatic Clustering" subtitle="Run clustering algorithm on all addresses">
          <div className="space-y-4">
            <p className="text-sm text-gray-600">
              Run the common-input heuristic clustering algorithm to automatically group related addresses.
            </p>
            <Button onClick={() => runClustering.mutate()} loading={runClustering.isPending} className="w-full">
        ...
        <Card title="Manual Clustering" subtitle="Group specific addresses into a cluster">
          <div className="space-y-4">
            ...
            />
            <Button onClick={handleManualCluster} loading={manualCluster.isPending} disabled={!manualClusterAddresses.trim()} className="w-full">
              <Users className="w-4 h-4 mr-2" />
              Create Cluster
            </Button>
          </div>
      ...
      <Card title="Tag Propagation" subtitle="Propagate risk tags through the graph">
        <div className="space-y-4">
          <Button onClick={() => propagateTags.mutate()} loading={propagateTags.isPending} variant="secondary" className="w-full">
